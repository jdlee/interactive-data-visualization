...
```{r minard, echo = FALSE, fig.cap = "Minards visualization of Napoleon's disasterous march to Moscow"}
...
ggplot(s.blood.df, aes(reorder(measure, -order), s.value)) +
  ...
  geom_text(aes(label = value), nudge_x = .3) +
...
## Visual perception for visualization
### Types of cognitive processes to be supported
Comparison
...
Popout effects and perceptual sensitivity
...
## Mapping typese of variables to graph features
...
mtcars.df$cyl = as.factor(mtcars.df$cyl)
cyl.order <- c("8", "6", "4")
 ggplot(mtcars.df, aes(x = cyl)) + 
   geom_bar()+
...
ggplot() +
  geom_line(data = belts.df, aes(x = Year, y = belts.DriversKilled, group = Month), alpha = .6) +
...
ggplot(names.df, aes(x = year, y = nn, group = name, fill = name)) +
  stat_steamgraph()+
  xlab(NULL) + 
  ylab(NULL) + 
  theme(legend.position = "none")
