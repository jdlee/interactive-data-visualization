...
library(DT)
sub.mtcars.df = mtcars.df %>% dplyr::select(mpg, wt) %>% 
...
mtcars.df = mtcars.df %>% mutate(gpm = 1/mpg) %>% mutate(speed = 1/qsec) %>% 
  select(-mpg, -qsec) %>% 
  select(cyl:carb, gpm, speed, name)
...
layout = G20.df %>% select(country, x, y, radius)
...
choropleth.df = choropleth.df %>% select(state:rate)
